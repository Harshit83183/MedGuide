  ...
  const finish = async () => {
    ...
    try {
      ...
      const rec = await api<{ id: number }>('/api/health-records', {
        method: 'POST',
        body: { user_id: user.id, title: p.title + ' — ' + days + ' din, ' + severity + '/10', issue: p.title + ' (' + p.titleHi + '). Pattern: ' + (pattern || 'n/a') + '. Flags: ' + (flags.join(', ') || 'none'), triage: r.level, severity, days: String(days), language: 'hinglish', advice: r.reasons.join(' | '), source: 'common-problems' },
      });
      sessionStorage.setItem('medguide_intake', JSON.stringify({ recordId: rec.id, text: p.title + ' — ' + pattern, lang: 'hinglish', days, severity, flags, problemKey: p.key }));
      nav('/triage');
    } catch (e) {
        ...
        {!p ? (
              ...
              return (
                <Reveal key={c.key} delay={(i % 4) * 0.07}>
                  <motion.button whileHover={{ y: -6 }} whileTap={{ scale: 0.97 }} onClick={() => { setSel(c.key); setStep(0); setFlags([]); setPattern(''); }} className="group block w-full rounded-3xl bg-white p-5 text-left shadow-sm ring-1 ring-slate-100 transition hover:shadow-xl hover:ring-teal-100">
                    ...
                    <p className="mt-3 font-extrabold text-[#0B1F3A]">{c.title} <span className="font-semibold text-slate-400">· {c.titleHi}</span></p>
                    <p className="mt-1 min-h-[36px] text-[13px] text-slate-500">{c.tagline}</p>
          ...
          <motion.div key="flow" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} className="mx-auto max-w-2xl">
            ...
            <div className="rounded-3xl bg-white p-6 shadow-xl ring-1 ring-slate-100 sm:p-8">
              <p className="text-xs font-bold uppercase tracking-widest text-teal-600">{p.title} · {p.titleHi}</p>
              ...
              <AnimatePresence mode="wait">
                {step === 0 && (
                  <motion.div key="s0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="pt-5">
                    <h3 className="text-lg font-extrabold text-[#0B1F3A]">Kitne din se {p.titleHi.toLowerCase()} hai?</h3>
                    <p className="text-sm text-slate-500">How many days has this lasted?</p>
                ...
                {step === 2 && (
                  <motion.div key="s2" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="pt-5">
                    <h3 className="text-lg font-extrabold text-[#0B1F3A]">{p.questions[2].qHi}</h3>
                    <p className="text-sm text-slate-500">{p.questions[2].q}</p>
                    <div className="mt-4 grid gap-2">
                      {(p.questions[2].choices || []).map((c) => (
                        <motion.button key={c} whileTap={{ scale: 0.98 }} onClick={() => setPattern(c)} className={'rounded-2xl border-2 px-4 py-3 text-left text-sm font-bold transition ' + (pattern === c ? 'border-teal-500 bg-teal-50 text-teal-800' : 'border-slate-200 text-slate-600 hover:border-teal-300')}>{c}</motion.button>
                      ))}
                    </div>
                  </motion.div>
                )}
                    ...
                    <div className="mt-4 grid gap-2 sm:grid-cols-2">
                      {RED_FLAGS.map((f) => (
                        <button key={f.key} onClick={() => toggleFlag(f.key)} className={'rounded-2xl border-2 px-3 py-2.5 text-left text-[13px] font-bold transition ' + (flags.includes(f.key) ? 'border-red-500 bg-red-50 text-red-700' : 'border-slate-200 text-slate-600 hover:border-red-300')}>
                          {flags.includes(f.key) ? '☑ ' : '☐ '}{f.labelHi}
                        </button>
                      ))}
                    </div>
                  </motion.div>
